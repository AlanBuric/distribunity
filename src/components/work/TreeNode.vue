        ...
        <template v-for="(inventory, index) in props.inventories" :key="index">
            <li v-if="!isLeaf(inventory)">
                <details open>
                    ...
                    <TreeNode :inventories="inventory.inventories" :selected-inventory="selectedInventory"
                        @select-inventory="(payload) => bubbleEvent('selectInventory', payload)" />
                </details>
            </li>
            <li v-else @click="mouseEvent => handleInventoryClick(mouseEvent, inventory)"
                :class="{ 'underlined': isSelected(inventory) }" class="clickable">{{
                    inventory.name }}</li>
        </template>
    </ul>
</template>