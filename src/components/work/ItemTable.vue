<script setup>
    ...
    const setActiveRow = (id) => (activeRow.value = id);
    ...
    <main>
        <table class="table-rounded">
            <tr v-if="props.inventory.items.length == 0">
                <td>
                    This inventory is currently empty. Use the "New" button to create a new item!
                </td>
            </tr>
            <template v-else>
                <tr>
                    <th v-for="columnName in columnNames" :key="columnName" @click="sortBy(Object.keys(columnName)[0])">
                        {{ Object.values(columnName)[0] }}
                    </th>
                </tr>
                <tr v-for="item in props.inventory.items" :key="item.id"
                    :id="item.id == activeRow ? 'active-item-row' : null" @click="setActiveRow(item.id)">
